...
let post_blind amount player =
  ...
  ({ player with chips = remaining_chips; round_bet = committed; status }, committed)
...
let commit_chips player amount =
  ...
  ({ player with chips; round_bet = player.round_bet + committed; status }, committed)
...
let advance_after_action game =
  ...
  if List.length remaining_players = 1 then
    ...
    Betting_round_complete game
  else
    let next_index =
      next_active_index game.players ((game.table.turn_index + 1) mod List.length game.players)
    in
    match next_index with
    | None -> Betting_round_complete game
    | Some turn_index -> Next_turn { game with table = { game.table with turn_index } }
...
let start players =
  ...
  let turn_index =
    match next_active_index players ((big_blind_index + 1) mod player_count) with
    | Some index -> index
