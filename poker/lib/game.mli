...
(** [award_pot game ~winner_id] moves the current pot into the winner's stack
    and zeroes the pot. Requires: [winner_id] names a player in [game] if the
    pot should be paid out. *)
val award_pot : Types.game_state -> winner_id:int -> Types.game_state

(** [next_hand game] starts the next hand using only surviving players and
    rotates the dealer button through the previous seating order. Returns [None]
    if zero or one player has chips left. Requires: surviving player ids are
    unique and [game.table.dealer_index] is valid for [game.players]. Raises:
