...
let broadcast_lobby state =
  let snapshot =
    match state.host_id with
    | Some id -> Poker.Lobby.snapshot ~host_id:id state.lobby
    | None -> Poker.Lobby.snapshot state.lobby
  in
...
(* Only emit a reveal message when the street actually changed; otherwise
   every post-flop action would re-broadcast "Flop revealed." and clobber
   the recent-action log on the client. *)
let reveal_message_for_street ?previous (game : Poker.Types.game_state) =
  let street_changed =
    match previous with
    | None -> true
    | Some prev -> prev.Poker.Types.table.street <> game.table.street
  in
  if not street_changed then None
  else Poker.Server_messages.reveal_message_for_street game
  ...
  | Some game -> (
      ...
      | Ok outcome -> (
          ...
          | Poker.Game.Next_turn next_game ->
              ...
              let%lwt () =
                match reveal_message_for_street ~previous:game next_game with
                | None -> Lwt.return_unit
                | Some message ->
...
let handle_start_game state player_id =
  ...
  if state.game <> None then
    ...
    Lwt.return state
  else if joined_client_count state < Poker.Protocol.min_players_to_start then
    let%lwt () =
      send_error
        (Printf.sprintf "Need at least %d players to start."
           Poker.Protocol.min_players_to_start)
    in
    Lwt.return state
